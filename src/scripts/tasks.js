...
const tasksDOM = (() => {
      ...
      todo.addEventListener("click", (e) => {
        if (e.target.closest(".btn-edit") || e.target.closest(".btn-delete")) {
          e.target.closest(".btn-delete")
            ? deleteTask(todo.getAttribute("id").substring(2))
            : openModalEdit(todo.getAttribute("id").substring(2));
        } else {
          toggleCheck(todo);
        }
      });
  ...
  const openModalEdit = (taskId) => {
    const modalContent = new newEditTask(
      taskId,
      Folder.allFolders.get(state.active).todos[taskId]
    );
    modal.open(
      modalContent.title,
  ...
  const toggleCheck = (todo) => {
    ...
    Folder.allFolders
      .get(state.active)
      .todos[todo.getAttribute("id").substring(2)].toggleChecked();
  };
  ...
  const tasksHtml = () => {
    ...
    if (
      Folder.allFolders.get(state.active) &&
      Folder.allFolders.get(state.active).todos.length > 0
    ) {
      Folder.allFolders.get(state.active).todos.forEach((todo, index) => {
        const htmlEl = `
                ...
                <div class="todo-main">
                    ...
                    <div class="todo-info">
                        <div class="todo-data">
                            <div class="todo-date"><i data-feather="calendar" class="small"></i>${
                              todo.dueDate
                            }</div>
                            <div class="todo-priority">
                                <div class="badge badge-${todo.priority.toLowerCase()}"></div> ${
          todo.priority
        }
                            </div>
                        </div>
                        <div class="todo-actions">
                            ...
                            <div class="todo-edit todo-action no-select btn-delete" id="${Folder.allFolders
                              .get(state.active)
                              .todos.indexOf(todo)}">
